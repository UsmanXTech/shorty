...
func TestQueryStoreSummary(t *testing.T) {
	...
	for _, e := range events {
		_, err = db.Exec(`INSERT INTO click_events (slug, created_at, referrer, user_agent, country) VALUES (?, ?, ?, ?, ?)`, "abc", e.at.Format(time.RFC3339Nano), e.ref, e.ua, e.country)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	if len(got.Buckets) != 2 || got.Buckets[0].Clicks != 2 || got.Buckets[1].Clicks != 1 {
		t.Fatalf("unexpected buckets: %+v", got.Buckets)
	}
	if got.Referrers[0].Name != "https://google.com" || got.Referrers[0].Clicks != 2 {
		t.Fatalf("unexpected referrers: %+v", got.Referrers)
	}
	if got.Countries[0].Name != "PK" || got.Countries[0].Clicks != 2 {
		t.Fatalf("unexpected countries: %+v", got.Countries)
	}
	if got.Browsers[0].Name != "Chrome" || got.Browsers[0].Clicks != 2 {
		t.Fatalf("unexpected browsers: %+v", got.Browsers)
	}
	if got.Devices[0].Name != "Desktop" || got.Devices[0].Clicks != 2 {
...
func TestClassifyUserAgent(t *testing.T) {
	...
	for _, tc := range cases {
		browser, device := classifyUserAgent(tc.ua)
		if browser != tc.browser || device != tc.device {
			t.Errorf("classify(%q) = %q/%q, want %q/%q", tc.ua, browser, device, tc.browser, tc.device)
		}
	}
}
