...
func main() {
	...
	switch os.Args[1] {
	case "create": create(client, os.Args[2:])
	case "stats": stats(client, os.Args[2:])
...
func create(client *cli.Client, args []string) {
	...
	if *expires != "" {
		...
		if err != nil { fmt.Fprintln(os.Stderr, "create: --expires-at must be RFC3339"); os.Exit(2) }
		link.ExpiresAt = &t
	}
...
func stats(client *cli.Client, args []string) {
	...
	parse := func(raw string) time.Time {
		...
		if err != nil { fmt.Fprintln(os.Stderr, "stats: dates must be RFC3339"); os.Exit(2) }
		return t
	}
